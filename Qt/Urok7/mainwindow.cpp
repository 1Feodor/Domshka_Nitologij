    ...
    , ui(new Ui::MainWindow)
{
    ...
    MyWindow = new MainWindowChart(this);

    connect(this, &MainWindow::sigResultReady, this, [this]{
       MyWindow->ViewGraph();
    });
}
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                for(int i = 0; i <= FD; ++i)
                                                {
                                                    X_Data.push_back(ind++ / 1000.0);
                                                }
                                                MyWindow->graphClass->ClearGraph(MyWindow->GetChart());
                                                MyWindow->graphClass->AddDataToGrahp(X_Data, res, 0);

                                                switch (ui->cmB_numCh->currentIndex()) {
                                                case 0:
                                                    MyWindow->GetAxisY()->setLabelFormat("%.2f");
                                                    MyWindow->GetAxisY()->setRange(-0.96, -0.8);
                                                    MyWindow->graphClass->SetNameSeries(1);
                                                    break;
                                                case 1:
                                                    MyWindow->GetAxisY()->setLabelFormat("%.3f");
                                                    MyWindow->GetAxisY()->setRange(0.031, 0.039);
                                                    MyWindow->graphClass->SetNameSeries(2);
                                                    break;
                                                case 2:
                                                    MyWindow->GetAxisY()->setLabelFormat("%.3f");
                                                    MyWindow->GetAxisY()->setRange(0.029, 0.037);
                                                    MyWindow->graphClass->SetNameSeries(3);
                                                    break;
                                                default:
                                                    break;
                                                }
                                                MyWindow->graphClass->UpdateGraph(MyWindow->GetChart());
                                                emit sigResultReady();
                                                /*
