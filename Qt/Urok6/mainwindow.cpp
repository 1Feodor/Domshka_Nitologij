...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto work1 = [this]{return concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};
        auto work2 = [this]{return concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};

        QFuture<void> fut = QtConcurrent::run(work1).then(work2);
    }
    else{
